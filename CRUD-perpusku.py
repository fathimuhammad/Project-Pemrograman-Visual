...
class perpusku(object):
    def __init__(self):
        self.window = Tk()        
        self.window.title('Aplikasi Perpustakaan')
    ...
    def db_name(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS perpus (
    ...
    def add_data(self):
        ...
        ListLabel = ('Nama Buku', 'Penulis', 'Penerbit', 'Tahun Terbit')
        ...
        def btn_save():
            query = 'INSERT INTO perpus VALUES(NULL, ?, ?, ?, ?)'            
            parameters = (text1.get(), text2.get(), text3.get(), textG.get())
            tkMessageBox.showinfo(
                'Berhasil',
                'Data perpus berhasil disimpan'
            )            
    ...
    def get_data(self):
        ...
        self.tree['columns'] = ('Nama Buku', 'Penulis', 'Penerbit', 'Tahun Terbit')
        self.tree.heading('#0', text='Nama Buku')
        self.tree.heading('#1', text='Penulis')
        self.tree.heading('#2', text='Penerbit')
        self.tree.heading('#3', text='Tahun Terbit')
    ...
    def update_data(self):
        ...
        try:
            ...
            return
        iddata = int(item[0])
        query = "SELECT * FROM perpus WHERE id = ? ;"
        self.selected_data(query, (iddata, ))
        result = cursor.fetchall()

        ListLabel = ('Nama Buku', 'Penulis', 'Penerbit', 'Tahun Terbit')        
        for i, l in enumerate(ListLabel):            
            globals()['label%s' % i] = Label(self.frame1, text=l).grid(row=i, column=0, sticky=S)

        text1 = StringVar(value=result[0][1])
        text2 = StringVar(value=result[0][2])
        text3 = StringVar(value=result[0][3])
        textG = IntVar(value=result[0][4])
        ...
        def btn_save():
            query = '''UPDATE perpus SET namabuku = ?, penulis = ?, penerbit = ?, tahunterbit = ? WHERE id = ?'''
            parameters = (text1.get(), text2.get(), text3.get(), textG.get(), iddata)
            tkMessageBox.showinfo(
                'Berhasil',
                'Data perpus berhasil diperbarui'
            )            
            self.selected_data(query, parameters)            
    ...
    def delete_data(self):
        ...
        try:
            if item is not None:
                ...
                if apakah == 'yes':
                    iddata = int(item[0])                
                    query = "DELETE FROM perpus WHERE id = ? ;"
                    self.selected_data(query, (iddata, ))
                    tkMessageBox.showinfo(
                        'Berhasil',
